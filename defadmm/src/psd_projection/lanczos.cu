...
// Seed of the Lanczos start vector.  If DEFADMM_LANCZOS_SEED is set, call c of this process uses
// seed = base + c (distinct per call, identical across runs); otherwise make_seed() (random_device
// + clock, not reproducible) is used.
static unsigned long lanczos_seed() {
    static const char* env = std::getenv("DEFADMM_LANCZOS_SEED");
    static const unsigned long base = env ? std::strtoul(env, nullptr, 10) : 0UL;
    static unsigned long calls = 0;
    return env ? base + (calls++) : make_seed();
}
...
void approximate_two_norm(
    ...
    /* Initial vector */
    // q = randn(n, 1)
    fill_random(q, n, lanczos_seed());   // reproducible when DEFADMM_LANCZOS_SEED is set
